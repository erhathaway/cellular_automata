const neighborRule = (ruleNumber, neighborhood) => {
  /* example:
...
const ruleObject = (ruleNumber) => {
  ...
  */
  const neighborhoods = [...Array(8).keys()]; // [0,1,2,3,4,5,6,7] the eight possible neighborhoods
  return neighborhoods.reduce(
    (acc, state) => {
      acc[state] = neighborRule(ruleNumber, state);
      return acc;
    },
  {});
}
