...
const createCoordinateExtractors = (serializedCoordinates) => {
  return serializedCoordinates.map((coordinate) => {
    ...
    if (coordinateExtractors.length === 1) {
      return ({ x }, arr) => coordinateExtractors[0](x, arr);
    }
    if (coordinateExtractors.length === 2) {
      return ({ x, y }, arr) => coordinateExtractors[1](y, coordinateExtractors[0](x, arr));
    }
    if (coordinateExtractors.length === 3) {
      return ({ x, y, z }, arr) => coordinateExtractors[2](z, coordinateExtractors[1](y, coordinateExtractors[0](x, arr))); // eslint-disable-line max-len
    }
    return () => { throw new ErrorCreatingCoordinateExtractor(); };
  });
};
