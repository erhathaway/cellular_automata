...
const Container = styled("div")`
  ...
  flex-direction: ${({ orientation }) =>
    orientation === "landscape" ? "row-reverse" : "column-reverse"};
`;
...
const Component = ({
  automataMenuStore: menu,
  deviceStateStore: device,
  routerStore: router,
  ...props
}) => {
  let shouldPositionPopup;
  if (menu.placement === "left") {
    shouldPositionPopup = "right";
  } else if (menu.placement === "right") {
    shouldPositionPopup = "left";
  } else if (menu.placement === "top") {
    shouldPositionPopup = "bottom";
  }
  ...
  return (
    <Container orientation={device.orientation}>
      ...
      >
        {state =>
          state !== "exited" && (
            <LibraryView transitionState={state} {...props} />
          )
        }
      </Transition>
      <ViewerAreaContainer id={VIEW_AREA_CONTAINER_ID}>
        ...
        <MenuContainer
          shouldPositionPopup={shouldPositionPopup}
          parentID={VIEW_AREA_CONTAINER_ID}
          {...props}
        >
...
export default inject("automataMenuStore", "deviceStateStore", "routerStore")(
  observer(Component)
);
